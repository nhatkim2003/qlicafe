...
public class BlankPlotLineChart extends JComponent {
    ...
    public void setMaxValues(double maxValues) {
        ...
        niceScaleLineChart.setMax(maxValues);
        repaint();
    }
    ...
    private NiceScaleLineChart niceScaleLineChart;
    ...
    public void initValues(double minValues, double maxValues) {
        ...
        this.maxValues = maxValues;
        niceScaleLineChart = new NiceScaleLineChart(minValues, maxValues);
        repaint();
    }
    ...
    private void createLine(Graphics2D g2) {
        ...
        double space = height / niceScaleLineChart.getMaxTicks();
        double locationY = insets.bottom + textHeight;
        double textWidth = getMaxValuesTextWidth(g2);
        double spaceText = 5;
        for (int i = 0; i <= niceScaleLineChart.getMaxTicks(); i++) {
    ...
    private void createValues(Graphics2D g2) {
        ...
        double height = getHeight() - (insets.top + insets.bottom) - textHeight;
        double space = height / niceScaleLineChart.getMaxTicks();
        double valuesCount = niceScaleLineChart.getNiceMin();
        double locationY = insets.bottom + textHeight;
        ...
        for (int i = 0; i <= niceScaleLineChart.getMaxTicks(); i++) {
            ...
            locationY += space;
            valuesCount += niceScaleLineChart.getTickSpacing();
        }
    }
    ...
    public SeriesSizeLineChart getRectangle(int index, double height, double space, double startX, double startY) {
        double x = startX + space * index;
        SeriesSizeLineChart size = new SeriesSizeLineChart(x, startY + 1, space, height);
        return size;
    }

    public NiceScaleLineChart getNiceScale() {
        return niceScaleLineChart;
    }

    public void setNiceScale(NiceScaleLineChart niceScaleLineChart) {
        this.niceScaleLineChart = niceScaleLineChart;
    }
